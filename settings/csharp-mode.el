...
;; Modification:
;;   Date   : $Date: 2013/03/ 6 17:13:26 CET$
;;   by     : F. Huber
;;   Comment: Curled bracket is added on a new line per default when a new 
;;            scope starts. This feature is removed and the { is added where
;;            it is typed.
              ...
              (let ((curline (line-number-at-pos))
                    (aftline (progn
                               (if (c-safe (backward-sexp) t)
                                 ...
                                 -1))))
                (= curline aftline)))
	    ;; Modification: $Date: 2013/03/ 6 17:11:21 CET$
	    ;; Comment: The original style added a new line, then the {. This
	    ;;          is to me ugly, I prefer to have the { on the same line
	    ;;          hence the code is changed from:
	    ;; Org Code start:
            ;;    (newline-and-indent)
	    ;; Org Code end.
	    ;; New code takes the form:
	    ()	  
	  )
        (self-insert-command 1)
